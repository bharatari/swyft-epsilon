...
export default Ember.Route.extend(AdminRouteMixin, { 
    model: function(params) {
        var url = "";
        if(params.id == 0) {
            url = config.routeLocation + '/api/admin/token/model';
        } 
        else {
            url = config.routeLocation + '/api/admin/token/' + params.id;
        }
        return Ember.RSVP.hash({
            ...
            metadata:  Ember.$.getJSON(config.routeLocation + '/api/admin/token/metadata', {token: JSON.parse(localStorage.getItem(loginUtils.localStorageKey)).token.token, tokenId: JSON.parse(localStorage.getItem(loginUtils.localStorageKey)).token.id}),
            id: params.id
        });
    },
