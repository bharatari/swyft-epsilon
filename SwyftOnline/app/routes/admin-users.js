...
export default Ember.Route.extend(AdminRouteMixin, {
    ...
    model: function(params) {
        return Ember.RSVP.hash({
            ...
            metadata:  Ember.$.getJSON(config.routeLocation + '/api/admin/user/metadata', {token: JSON.parse(localStorage.getItem(loginUtils.localStorageKey)).token.token, tokenId: JSON.parse(localStorage.getItem(loginUtils.localStorageKey)).token.id, limit: 100, where: params.filters })
        });
    },
