...
export default Ember.Route.extend(AdminRouteMixin, {
    ...
    model: function(params) {
        return Ember.RSVP.hash({
            orders: Ember.$.getJSON(config.routeLocation + '/api/admin/order', {token: JSON.parse(localStorage.getItem(loginUtils.localStorageKey)).token.token, tokenId: JSON.parse(localStorage.getItem(loginUtils.localStorageKey)).token.id, sort: params.sort + " " + params.sortType, skip: (params.page - 1) * 100, where: params.filters, limit: 100 }),
