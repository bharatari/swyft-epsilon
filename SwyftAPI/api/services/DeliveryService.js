...
module.exports = {
    ...
    setDeliveryOffset: function(reqOffset, deliveryId, cb) {
        ...
        Delivery.findOne({ id: deliveryId }).exec(function(err, delivery) {
            ...
            if (!err || delivery) {
                Delivery.update({ id: deliveryId }, { estimatedArrival: self.processArrivalOffset(new Date(delivery.scheduledArrival), offset), estimatedDelivery: self.processDeliveryOffset(new Date(delivery.deliveryDate), offset) }).exec(function(err, data) {
                    if (err) {
                        return cb(true);
                    } else {
